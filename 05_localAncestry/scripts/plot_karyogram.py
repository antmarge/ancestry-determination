...
parser.add_argument('--pop_order', default='Pop1,Pop2,Pop3,Pop4,Pop5', 
...
def plot_rects(anc, chr, start, stop, hap, pop_order, colors, chrX):    
    ...
    if int(anc) <= len(pop_order) + 1:
        col=mcol.PathCollection([clip_path],facecolor=colors[int(anc)], linewidths=0)
    else:
...
bmap = brewer2mpl.get_map('Set1', 'qualitative', len(pop_order))
